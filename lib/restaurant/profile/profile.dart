...
import '../signin/token_manager.dart';


class Profile extends StatefulWidget {
  const Profile({required this.name, required this.token, Key? key}) : super(key: key);
  final String token;
  final String name;
...
class ProfileState extends State<Profile> {
  ...
  void fetchProfileData() async {
    try {
      ...
      final response = await http.post(
        Uri.parse("http://192.168.7.91:8080/signin/restaurant/profile"),
  ...
  void _onItemTapped(int index) {
    ...
    if (index == 1) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => menu(name: widget.name, token: widget.token)),
      );
    } else if (index == 2) {
      // Navigate to Reservations screen
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => reservations(name: widget.name, token: widget.token)),
      );
    }
  }
  ...
  void _updateProfile() async {
    ...
    if (_addressController.text != _profileData?["address"]) {
    ...
    try {
      final response = await http.put(
        Uri.parse("http://192.168.7.91:8080/signin/restaurant/update_profile"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        body: AnimatedSwitcher(
          ...
          child: _isEditing ? _buildEditingView() : _dataVisible ? _buildDataView() : _buildLoadingView(),
        ),
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(_profileData!["email"],style: const TextStyle(color: Colors.white),),
            const SizedBox(height: 8),
            const Hero(
            ...
            Text(_profileData!["password"],style: const TextStyle(color: Colors.white),),
            const SizedBox(height: 8),
            const Hero(
              tag: 'profile-address',
              child: Material(
            ...
            Text(_profileData!["address"],style: const TextStyle(color: Colors.white),),
